...
本仓库当前处于客户 Alpha 收口阶段。MVP 1 的 Rust core、CLI、stdio sidecar、`agentcafe doctor --json`、`DiagnosticReport` schema、fixture、redaction、安全边界测试，以及 macOS SwiftUI / Windows WPF 只读 UI shell 已经落地。

首个客户可测版本定位为受控 Alpha：只提供只读诊断、原生 UI 展示、trace id 复制、基础 retry 和脱敏诊断详情。MVP 2 写入、snapshot、restore、MCP 连接测试、Hook dry-run 和 Plugin 命令仍保持禁用。
...
- [Customer Alpha Release Plan](docs/customer-alpha-release-plan.md)
- [DiagnosticReport JSON Schema](schemas/diagnostic-report.schema.json)
- [ConfigDiff JSON Schema](schemas/config-diff.schema.json)
- [ConfigApply JSON Schema](schemas/config-apply.schema.json)
- [SnapshotManifest JSON Schema](schemas/snapshot-manifest.schema.json)
...
agentcafe/
  ...
  docs/
    ...
    customer-alpha-release-plan.md
  schemas/
